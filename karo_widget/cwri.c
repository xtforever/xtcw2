...
/* --------------------------------------------------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------------------------------------------------- */
void sr_lookup( Widget w, void *u, void *c )
{
    
    char *s;
    XtVaGetValues( w, "label", &s, NULL );
    TRACE(1,"sr_lookup %s", s );
}

void forward( Widget w, void *u, void *c )
{
    TRACE(1,"");
    struct mv_data* mv;
    int q = XrmStringToQuark("QPS");
    mv = mv_read_struct(q);
    s_printf(mv->data,0,"%d", 76543 );
    XtVaSetValues(CWRI.form, "label", "test", NULL );
    
}
void backward( Widget w, void *u, void *c )
{
...
static void InitializeApplication( Widget top )
{
    ...
    for(int i=0; i< ALEN(s); i++ ) {
      struct mv_data* mv;
      int q = XrmStringToQuark(s[i]);
      mv = mv_read_struct(q);
      mv->data = s_printf(0,0,"%d", i );
      mv->type = "MSTR";
    }
