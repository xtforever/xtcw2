...
XtNpercent&XtCPercent&int &"75"\cr
XtNgauge{\underline}width&XtCGauge{\underline}width&Distance &"5mm"\cr
XtNhh&XtCHh&Distance &"2mm"\cr
XtNresource&XtCResource&String &"none"\cr
\endTable
\Section
\Publicvar{XtNlabel}
...
\Section
\Publicvar{XtNpercent}
int  percent = {\langle}String{\rangle}"75"

\Section
\Publicvar{XtNgauge{\underline}width}
Distance  gauge{\underline}width = {\langle}String{\rangle}"5mm"

\Section
\Publicvar{XtNhh}
Distance  hh = {\langle}String{\rangle}"2mm"

\Section
\Publicvar{XtNresource}
String  resource = {\langle}String{\rangle}"none"
...
{\lbrace}
    ...
    float wx = {\dollar}width;
        wx /= 100.0;
       wx *= {\dollar}percent;
       if( wx {\rangle} {\dollar}width ) wx = {\dollar}width;
       
       /*   int wx = {\dollar}gauge{\underline}width; */

    XftDrawRect({\dollar}draw,{\dollar}xft{\underline}col + {\dollar}state,0,0,wx,{\dollar}height);
...
{\lbrace}
    ...
    XtResource dp;
    dp.resource{\underline}name = {\dollar}resource;
    dp.resource{\underline}class = 0;
    dp.resource{\underline}type = XtRString;
    dp.resource{\underline}size = sizeof(char*);
    dp.resource{\underline}offset = 0;
    dp.default{\underline}type = XtRString;
    dp.default{\underline}addr = NULL;

    TRACE(1,"{\percent}s: {\percent}s", {\dollar}name, {\dollar}resource );

    /*
    XrmName xrm{\underline}name[2];
    XrmClass xrm{\underline}class[2];
    XrmRepresentation rep{\underline}type;
    XrmValue value;
    xrm{\underline}name[0] = XrmPermStringToQuark( "testpfad" );
    xrm{\underline}class[0] = XrmPermStringToQuark( "Testpfad" );
    xrm{\underline}name[1] = NULLQUARK;
    xrm{\underline}class[1] = NULLQUARK; 
    */

    XrmValue value;
    XrmDatabase db = XrmGetDatabase( XtDisplay({\dollar}) );
    if(!db) XGetDefault(XtDisplay({\dollar}), "", "" );
    db = XrmGetDatabase( XtDisplay({\dollar}) );
    char *type{\underline}ret=0;
    XrmGetResource( db, "cwri.testpfad", "Int", {\ampersand}type{\underline}ret, {\ampersand}value);
    
    
    TRACE(1,"{\percent}s: {\percent}s {\percent}s", {\dollar}name, value.addr, type{\underline}ret );
     /*
     XtGetApplicationResources( {\dollar}TopLevel, (XtPointer) {\ampersand} ({\dollar}fromResource),
                               {\ampersand}dp, 1, NULL, 0 );

     */   
    

    if( !is{\underline}empty({\dollar}fromResource) ) {\lbrace}
        TRACE(1,"{\percent}s: {\percent}s", {\dollar}name, {\dollar}fromResource );       
        int x = atoi({\dollar}fromResource);
        if( x != {\dollar}percent ) {\lbrace} {\dollar}percent = x; redraw({\dollar}); {\rbrace}
    {\rbrace}
